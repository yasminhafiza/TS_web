<template>
  <section id="clients" class="relative w-full py-16 md:py-24 lg:py-32 bg-white dark:bg-[#000839] overflow-hidden transition-colors duration-500">
    ...
    <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6">
      ...
      <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 md:gap-8 mb-12 md:mb-16 gsap-header" ref="headerRef">
        <div class="max-w-2xl text-left">
          <div class="flex items-center gap-3 mb-3 md:mb-4">
            <span class="w-8 md:w-12 h-[1px] bg-[#ff1e42]"></span>
          </div>
          <h2 class="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black text-slate-800 dark:text-white leading-[1.1] md:leading-none tracking-tighter">
            OUR <br class="hidden md:block"/>
            <span class="text-transparent bg-clip-text bg-gradient-to-r from-[#ff1e42] to-[#ff8095]">CLIENTS.</span>
          </h2>
        </div>
        <p class="text-slate-500 dark:text-slate-300 max-w-xs text-xs md:text-sm leading-relaxed border-l border-slate-200 dark:border-white/10 pl-4 md:pl-6 uppercase tracking-wider font-medium">
          Collaborating with industry leaders to deliver high-level operational excellence and innovation.
        </p>
      </div>

      <div class="relative flex overflow-hidden py-8 md:py-12 mask-fade">
        <div class="flex flex-nowrap gap-4 md:gap-6 animate-marquee items-center">
          <div 
            v-for="(client, index) in [...clients, ...clients]" 
            :key="index"
            class="flex-shrink-0 w-28 h-20 sm:w-32 sm:h-24 md:w-40 md:h-32 relative group flex items-center justify-center"
          >
            <div class="absolute inset-0 scale-75 bg-[#ff1e42]/10 blur-[20px] md:blur-[40px] opacity-0 group-hover:opacity-100 transition-opacity duration-700 rounded-full"></div>
            
            <div class="relative h-full w-full flex items-center justify-center p-3 md:p-4">
              <img 
                :src="client.logo" 
                :alt="client.name"
            ...
            <span class="absolute -bottom-1 md:-bottom-2 left-1/2 -translate-x-1/2 bg-slate-900 dark:bg-[#ff1e42] text-white text-[7px] md:text-[9px] font-bold px-2 md:px-3 py-1 rounded-full opacity-0 group-hover:opacity-100 transition-all duration-300 pointer-events-none uppercase tracking-widest z-20 shadow-xl whitespace-nowrap">
              {{ client.name }}
            </span>
          </div>
        </div>
